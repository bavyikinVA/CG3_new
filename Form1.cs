...
namespace CG3
{
    public partial class Form1 : Form
    {
        ...
        SolidBrush back = new SolidBrush(Color.White);//заливка фона

        Pen penRed = new Pen(Color.Red, 3); //цвет линий координат
        Pen penGreen = new Pen(Color.Green, 5); //цвет параллельных прямых

        //Точки для линий
        Point[] lines = new Point[4] { new Point(1000, 200), new Point(1200, 300), new Point(1000, 250), new Point(1200, 350) };

        //Функция прорисовки элементов в конктрукторе
        public void Draw()
        {
            g.FillRectangle(back, 0, 0, 1940, 1100);

            //рисуем линии координатных прямых
            g.DrawLine(penRed, 0, Height / 2, Width, Height / 2);
            g.DrawLine(penRed, Width / 2, 0, Width / 2, Height);

            //рисуем линии по координатам точек lines
            g.DrawLine(penGreen, lines[0], lines[1]);
            g.DrawLine(penGreen, lines[2], lines[3]);
        }
        ...
        public void TransformMatrix(ref Point[] lines, double turn, double transferX, double transferY, double stretchingX, double stretchingY, ref Point[] currentLines)
        {
            ...
            if (checkBoxTransfer.Checked) // галка сдвига
            {
            ...
            if (checkBoxStretching.Checked) // галка растяжения
            {
        ...
        {
            double turn = Int32.Parse(textBox3.Text);
            double TransferX = Int32.Parse(textBox4.Text);
            double TransferY = Int32.Parse(textBox5.Text);
            double StretchingX = Int32.Parse(textBox1.Text);
            double StretchingY = Int32.Parse(textBox2.Text);
